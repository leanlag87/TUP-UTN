...
try:
    with conexion:
        with conexion.cursor() as cursor:
            ...
            sentencia = 'SELECT * FROM persona WHERE id_persona  IN %s' # IN es una cláusula SQL que permite seleccionar registros que coincidan con una lista de valores
            entrada = input("Ingrese el ID de la persona que desea consultar (separados por comas): ") # Pedimos al usuario que ingrese el ID de la persona que desea consultar
            llaves_primarias = (tuple(entrada.split(',')),) # Convertimos la entrada en una tupla de llaves primarias
            #llaves_primarias = ((1, 2, 3),) # Definimos una lista de llaves primarias que queremos seleccionar
            ...
            cursor.execute(sentencia, llaves_primarias) # Ejecutamos la consulta
            ...
            for resultado in resultado:
                print(resultado) # Imprimimos cada resultado
